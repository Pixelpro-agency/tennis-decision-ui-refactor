...
Un valore CDP corrente vuoto usa il valore confermato quando disponibile. Il valore predefinito locale è usato soltanto per compatibilità legacy quando entrambi i valori CDP sono assenti.
...
`useLiveTrackingActions.js` coordina richiesta, stato UI e stop del polling SofaScore. `App.jsx` collega queste callback alla shell e al componente dashboard.
