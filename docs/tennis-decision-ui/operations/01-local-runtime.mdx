...
Il percorso ordinario è stato validato con il collaudo runtime `9A-R`:

```txt
avvio canonico
lock e manifest schema 2
riuso della sessione
seconda invocazione session_reuse
shutdown Ctrl+C
terminazione dei soli backend/frontend owned
rilascio lock
rimozione manifest
riavvio con nuove identità
...
Una sessione è riusabile soltanto quando lock, manifest schema 2 e identità dei servizi sono coerenti.
...
L’evento `browser_open` sul frontend esistente è corretto: riapre l’interfaccia senza acquisire ownership sui servizi già attivi.
...
Lo snapshot pubblico è disponibile tramite [API Runtime Health](../api/06-runtime-health.mdx). I lifecycle completi restano nei documenti SofaScore e Betfair.
