...
Questo documento descrive come i dati correnti attraversano acquisizione, normalizzazione, Source Identity, persistenza, lettura ed Evidence.
...
Il gate corrente e i tracker sono correlati principalmente tramite `eventId`. Non esiste ancora un identificatore di sessione propagato a callback, scraper, conferme e poller; questo limite non va confuso con una garanzia già presente.
...
Il sistema possiede già atomic write per singolo file, commit journal e recovery. Restano però limiti registrati:

- nessun lock esclusivo backend-owned prima di recovery e listener;
