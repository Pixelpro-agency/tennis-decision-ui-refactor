...
La route normalizza l’URL rimuovendo slash finali e verifica:

```txt
<cdpUrl>/json/version
```

Quando `cdpUrl` è assente o falsy, la route usa il fallback:

```txt
http://127.0.0.1:9222
```

Una stringa composta solo da spazi non usa questo fallback, perché è truthy prima della normalizzazione. I chiamanti devono quindi omettere il campo o inviare un URL valido.

La route non apre Chrome e non modifica la sessione browser.

Gli esiti gestiti restituiscono body JSON con status HTTP `200`, anche quando `ok` è `false`.
...
| ---------------------------------- | -------------------------------------------------------- |
| Risposta vuota                     | `ok: false`, `error: 'Empty response from CDP'`          |
| JSON non valido                    | `ok: false`, `error: 'Invalid JSON from CDP'`, `snippet` |
| HTTP non riuscito                  | `ok: false`, `status`, `error`                           |
| Endpoint raggiunto senza WebSocket | `ok: false`, `browser`, `error`                          |
| Endpoint irraggiungibile           | `ok: false`, `error`                                     |

La decisione del frontend di non inviare il preflight quando Chrome non è disponibile appartiene al modulo frontend. Questo router resta comunque invocabile e applica il fallback locale quando il campo CDP manca.

Il fallback CDP di questa route non modifica il contratto di:

```txt
POST /api/match/track
```

Il tracking in modalità CDP mantiene le proprie regole di validazione.
...
```txt
CDP senza cdpUrl
→ fallback locale 127.0.0.1:9222
